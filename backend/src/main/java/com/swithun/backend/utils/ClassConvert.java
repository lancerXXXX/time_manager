 ...
 * @LastEditTime: 2021-05-31 09:06:51
 */
...
public class ClassConvert {
    ...
    public List<Integer> getChildrenType(Integer typeId) {
        ...
        //     res.add(type.getId());
        // }
        getAllChildPlan(planType, res);
        return res;
    }

    void getAllChildPlan(PlanTypeEntity planType, Collection<Integer> res) {
        res.add(planType.getId());
        Collection<PlanTypeEntity> planTypes = planType.getPlanTypesById();
        if (planTypes != null) {
            for (PlanTypeEntity tempPlan : planTypes) {
                getAllChildPlan(tempPlan, res);
            }
        }
    }
